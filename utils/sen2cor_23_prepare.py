# coding=utf-8
"""
Prepare S2A USER MSI L2A data for ingestion into AGDCv2 from the command-line. Based on the original sen2cor_prepare.py from /datacube-core/utils
TODO: Include the classification layers AOT, WVP....)
"""
...
def prepare_dataset(path):
    ...
    for granule_id, images in granules.items():
        ...
        img_data_path_r10 = str(os.path.join('GRANULE', granule_id, 'IMG_DATA', 'R10m'))
        img_data_path_r20 = str(os.path.join('GRANULE', granule_id, 'IMG_DATA', 'R20m'))
        #img_data_path_r60 = str(path.parent.joinpath('GRANULE', granule_id, 'IMG_DATA', 'R60m'))


        for image in images:
            #classification_list = ['AOT']
